...
if __name__ == '__main__':
    ...
    with open(args[0]) as database:
        for i,row in enumerate(csv.reader(database)):
            if i > 0:
    ...
    int_itemset=[]
    classes = ['Iris-versicolor', 'Iris-versicolor','Iris-virginica']

    result = sorted(result, key=lambda i: i[1])
    for itemset, support in result:
        if (len(itemset) > 2 and (('Iris-setosa' in itemset) or ('Iris-versicolor' in itemset) or ('Iris-virginica' in itemset))):
            #print(str(itemset) + ' ' + str(support))
            int_itemset.append([itemset,support])

    with open(args[0]) as database:
        db = list(csv.reader(database))
        db = db[1:]
    itemset_dict = {}
    for item,support in int_itemset:
        ...
        for value in item:
            ...
            if value.startswith('Iris'):
                _class = value
        if _class not in itemset_dict:
            itemset_dict[_class] = []
        f_list = [x for x in [f1, f2, f3, f4] if x is not None]
        count = 0
        for i,row in enumerate(db):
            a = [1 for x in f_list if x in row]
            if len(a) == len(f_list):
                count=count + 1

        itemset_dict[_class].append([f_list,support,round(support/count,2)]) 

    for key in itemset_dict:
        for i,feat_vec in enumerate(itemset_dict[key]):
            itemset_dict[key][i][0] = sorted(itemset_dict[key][i][0], key=lambda v: sort_dict(v))
        itemset_dict[key] = sorted(itemset_dict[key], key=lambda v: sort_dict(v[0][0]))
        print(key)
        for rules in itemset_dict[key]:
            print('\t',rules)
    import pickle
    pickle.dump(itemset_dict, open('association_rules_' + str(options.minsup) + '.pkl', 'wb'))